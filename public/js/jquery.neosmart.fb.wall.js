		...
		return meta.each(function() {
			...
			var initWall = function(data){
				...
				for(var k=0;k<max;k++){
					...
					// Box -----------------------------------------------------------------------------------------------------------------------------------
					output += (k==0) ? '<div class="grid col-xs-12" class="fb-wall-box fb-wall-box-first">' : '<div class="grid col-xs-12 hidden-xs" class="fb-wall-box">';
					output += '<div class="borders"><div class="grid_content"><div class="pad"><a href="http://www.facebook.com/profile.php?id='+data[k].from.id+'" target="_blank">';
					output += '<img class="fb-wall-avatar" src="'+getAvatarURL(data[k].from.id)+'" />';
					output += '</a>';
					output += '<div class="fb-wall-data-container"><div class="fb-wall-data">';
					
					output += '<div class="fb-wall-message">';
					output += '<p><a href="http://www.facebook.com/profile.php?id='+data[k].from.id+'" class="fb-wall-message-from" target="_blank">'+data[k].from.name+'</a></p> ';
					...
					if(exists(data[k].picture)||exists(data[k].link)||exists(data[k].caption)||exists(data[k].description)){
						...
						if(exists(data[k].description)){
							...
							output += '<div class="fb-wall-description">'+thisDesc+'</div>';
						}
						output += '</div>';
						output += '</div>';
					}
					output += '<div class="fb-wall-date">';
					...
					if(exists(data[k].likes)){
						if(parseInt(data[k].likes.data.length)==1){
							output += '<div class="fb-wall-likes"><div><span>'+data[k].likes.data[0].name+'</span> さんが「いいね！」と言っています。</div> </div>';
						} else if(parseInt(data[k].likes.count)==2){
							output += '<div class="fb-wall-likes"><div><span>'+data[k].likes.data[0].name+'</span> さんと<span>'+data[k].likes.data[1].name+'</span> さんが「いいね！」と言っています。</div> </div>';
						} else {
							output += '<div class="fb-wall-likes"><div><span>'+data[k].likes.data[0].name+'</span> さんと<span>'+data[k].likes.data[1].name+'</span> さん、<span>他'+(data[k].likes.data.length - 2)+'人</span>が「いいね！」と言っています。</div></div>';
						}
					}
					...
					if(exists(data[k].comments) && exists(data[k].comments.data) && (o.showComments==true||o.showComments=='true')){
						...
						for(var c=0;c<data[k].comments.data.length;c++){
							output += '<div class="fb-wall-comment">';
							...
							output += '</a>';
							output += '<div class="fb-wall-comment-message">';
							...
							output += '<div class="fb-wall-comment-from-date">'+formatDate(data[k].comments.data[c].created_time)+'</div>';
							output += '</div>';
							output += '</div>';
						}
						output += '</div>';
					...
					output += '</div>';
					output += '<div class="fb-wall-clean"></div></div></div></div>';
					output += '<div class="clear_box"></div></div></div>';
				}
				...
				if(max==0){
					...
					output += '<div class="fb-wall-message"><p><div class="fb-wall-message-from">'+baseData.name+'</div></p> '+o.translateErrorNoData+'</div>';
					output += '</div>';
					output += '</div>';
				}
