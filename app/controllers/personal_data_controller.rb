...
# GET /tests/1/edit
  ...
  def update
    @personal_data = PersonalData.where(:id => current_user.id).first
    old_email = @personal_data.email
    old_email2 = @personal_data.email2
    ...
    respond_to do |format|
      if @personal_data.update(personal_data_params)
        ...
        #ml対応
        proc_ml(old_email, old_email2, @personal_data)
  ...
  def personal_data_params
      params.require(:personal_data).permit(:family_name, :name, :family_name_pron,
       :name_pron, :nickname, :part_id, :email, :email2, :phone, :zip_code, :address, :birth_date,
       :building, :worker, :school, :bank, :bank_branch, :bank_account, :disclosed_phone, :mobile_mail, :pc_mail, :job, :grad_school, :major, :favorite, :facebook, :twitter, :mixi, :github, :googleplus, :tumblr, :linkedin, :flickr, :ameba, :hatena, :line, :yelp, :yahoo, :bing, :blogger, :instagram, :behance, :origin, :birthday, :blood);
  end
